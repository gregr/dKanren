...
(define (wake-path path ss)
  (cond
    ((conj? ss) (conj (wake-path path (conj-c1 ss)) (conj-c2 ss)))
    ((disj? ss)
     (cond ((null? path) (error 'wake-path (format "path is too short ~s" ss)))
           ((car path) (disj (wake-path (cdr path) (disj-c1 ss)) (disj-c2 ss)))
           (else (disj (disj-c1 ss) (wake-path (cdr path) (disj-c2 ss))))))
    ((pause? ss)
     (cond ((not (zzz? (pause-goal ss)))
            (error 'wake-path (format "expected zzz ~s ~s" path ss)))
           ((pair? path)
            (error 'wake-path (format "path is too long ~s ~s" path ss)))
           (else (pause (pause-state ss) ((zzz-wake (pause-goal ss)))))))
...
(define (expand-path path path-expected ss0)
  (define ss (prune #t (wake-path path ss0)))
