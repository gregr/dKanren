...
(define (good-path hint ss)
  (let ((choices (labeled-solution* (inject-hint hint ss))))
    (when (null? choices) (error 'good-path "solution already found"))
    (caar (reverse choices))))

(define (follow/feedback good path ss)
  (define expansion (expand-path path good ss))
  (define flag (car expansion))
  (define ss2 (cadr expansion))
  (define pss (prune #t ss2))
  (define flag2
    (cond ((or (state? pss) (pair? pss))
           (if (or (eq? 'fail flag) (eq? 'fail-solved flag))
             'fail-solved
             'solved))
          ((not pss) 'fail)
          (else flag)))
    ...
    (when (not (eof-object? request))
      (cond
        ...
        ((and (pair? request) (valid-path? request))
         (let* ((result (follow/feedback good request ss))
                (flag (car result))
                (ss2 (cadr result)))
